<script lang="ts">
	...
	$: ({ data: profile } = profileDetails.select(userId)($store));
	$: ({ data: posts } = postsByProfile.select(userId)($store));
	$: ({ data: followers } = followersCount.select(userId)($store));
	$: ({ data: following } = leadersCount.select(userId)($store));
	$: ({ data: isFollowing } = isFollowingService.select(userId)($store));
	$: currentUser = $accountsStore.user;

	onMount(async () => {
		await store.dispatch(profileDetails.initiate(userId));
		await store.dispatch(postsByProfile.initiate(userId));
		await store.dispatch(isFollowingService.initiate(userId));
		await store.dispatch(followersCount.initiate(userId));
		await store.dispatch(leadersCount.initiate(userId));
	});

	async function followUnfollow() {
		await store.dispatch(followOrUnfollow.initiate(userId));
	}
</script>
